function sc_draw_menu_topscores(start_x, start_y) {
	if sel_menu = 4 { //top scores
		...
		var _vadd = 64;
		var _add = 0;
		draw_text(start_x + _add, start_y - 16 + topscores_adjust_y, "Place");
		_add += 512;
		_add -= 96;
		draw_set_halign(fa_right);
		draw_text(start_x + _add, start_y - 16 + topscores_adjust_y, "Score");
		_add += 256;
		_add += 128;
		_add -= 32;
		draw_set_halign(fa_center);
		draw_text(start_x + _add, start_y - 16 + topscores_adjust_y, "Character");
		_add -= 32;
		_add += 256;
		_add += 128;
		draw_set_halign(fa_left);
		draw_text(start_x + _add, start_y - 16 + topscores_adjust_y, "Threat");
		_add += 256;
		draw_text(start_x + _add, start_y - 16 + topscores_adjust_y, "Wave");
		_add += 256;
		draw_text(start_x + _add, start_y - 16 + topscores_adjust_y, "Upgrades");
		...
		for (var i = 0; i < 25; i++) {
			if top10_scores[1][i] != -1 {
				var _add = 0;
				draw_line(start_x - 128, start_y + _add_row + _vadd + 64*(i-.5) + topscores_adjust_y, display_get_gui_width() - start_x + 128, start_y + _add_row + _vadd + 64*(i-.5) + topscores_adjust_y);
				draw_text(start_x, start_y + _add_row + _vadd + 64*i + topscores_adjust_y, i+1);
				_add += 512;
				_add -= 96;
				draw_set_halign(fa_right);
				if top10_scores[0][i] != -1 {
					draw_text(start_x + _add, start_y + _add_row + _vadd + 64*i + topscores_adjust_y, top10_scores[0][i]);
				}
				_add += 256;
				_add += 128;
				_add -= 32;
				draw_set_halign(fa_center);
				if top10_scores[1][i] != -1 {
					if top10_scores[3][i] != -1 and color[top10_scores[3][i]][0] != "Blue" draw_text(start_x + _add, start_y + _add_row + _vadd + 64*i + topscores_adjust_y, string(color[top10_scores[3][i]][0]) + " " + string(character[top10_scores[1][i]][0]));
					//if 1=1 draw_text(start_x + _add, start_y + _add_row + _vadd + 64*i, "Orange Summoner");
					else draw_text(start_x + _add, start_y + _add_row + _vadd + 64*i + topscores_adjust_y, character[top10_scores[1][i]][0]);
				}
				_add -= 32;
				_add += 256;
				_add += 128;
				draw_set_halign(fa_left);
				if top10_scores[2][i] != -1 draw_text(start_x + _add, start_y + _add_row + _vadd + 64*i + topscores_adjust_y, top10_scores[2][i]);
				_add += 256;
				//wave
				if top10_scores[5][i] != -1 draw_text(start_x + _add, start_y + _add_row + _vadd + 64*i + topscores_adjust_y, top10_scores[5][i]);
				_add += 256;
					...
					for (var k = 1; k < string_length(_upgrade_string)+1; k++) {
						if string_char_at(_upgrade_string, k) != ":" {
							...
							if selected_leaderboard = i and selected_column = _total_upgrade_number-1 {
								draw_sprite_stretched_ext(s_upgrades, real(_current_upgrade)+1, start_x + _add + _add2, start_y + _add_row + _vadd + 64*i - 7*4 + topscores_adjust_y, 14*4, 14*4, c_white, 1);
								...
								draw_sprite_stretched_ext(s_upgrades, real(_current_upgrade), start_x + _add + _add2, start_y + _add_row + _vadd + 64*i - 7*4 + topscores_adjust_y, 14*4, 14*4, c_white, 1);
							}
						...
						if selected_leaderboard = i and selected_leaderboard != -1 {
							...
							if keyboard_check_pressed(global.key[ctrl.right]) || gamepad_button_check_pressed(0, gp_padr) {
								...
								if selected_column < 0 {
									...
									if selected_leaderboard < 0 selected_leaderboard = 24;
								}
								...
								if selected_column >= _total_upgrade_number {
									...
									selected_leaderboard++;
									if selected_leaderboard > 24 selected_leaderboard = 0;
								}
							}
						}
					}
		...
		draw_text(_upgrade_x, _upgrade_y + topscores_adjust_y, upgrade_name[_selected_upgrade_number]);
		draw_set_halign(fa_left);
	}
}