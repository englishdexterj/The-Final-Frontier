function sc_shotgunner(target) {
	if distance_to_object(target) >= 120*global.scale*target.size {
		forward = ((distance_to_object(target)-120*global.scale)/(60*global.scale))/4;
	} else if distance_to_object(target) <= 60*global.scale*target.size {
		forward = -((60*global.scale-distance_to_object(target))/(30*global.scale))*2;
	} else {
		forward = 0;
	}
	
	var ang_diff = sc_get_angdiff(target);
	
	if move_avoidance ang_diff = sc_move_avoidance(ang_diff, target, 1);
	else sc_shoot(ang_diff);
	
	acceleration = clamp((acceleration + forward*global.spd*.1)*(global.drag-.08), -1, 1);

	//move player
	x_speed += lengthdir_x(acceleration + abs(clamp(ang_diff/5,-.2, .2)), direction);
	y_speed += lengthdir_y(acceleration + abs(clamp(ang_diff/5,-.2, .2)), direction);
	
	turn_acceleration = clamp(turn_acceleration_mult*(ang_diff/5)/60, -turn_acc, turn_acc);
	turn_speed = clamp((turn_speed + turn_acceleration) * (global.fric - .08), -max_turn_speed, max_turn_speed);
	
	direction += turn_speed;
	image_angle = direction;
}