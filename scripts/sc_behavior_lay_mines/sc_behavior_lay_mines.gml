function sc_behavior_lay_mines(target) {
	//check for movement
	if distance_to_object(target) >= 400*global.scale*target.size {
		forward = ((distance_to_object(target)-400*global.scale)/(60*global.scale))/4;
	} else if distance_to_object(target) <= 200*global.scale*target.size {
		forward = -((200*global.scale-distance_to_object(target))/(30*global.scale))*2;
	} else {
		forward = 0;
	}

	var ang_diff = sc_get_angdiff(target);
	if ang_diff > 0 ang_diff -= 90
	else if ang_diff < 0 ang_diff += 90
